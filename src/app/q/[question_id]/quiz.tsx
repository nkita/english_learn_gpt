...
export default function Quiz({ description, content, type, level, max_count = 10, current_count = 6, q_status, q_id, inprogress }: { description: string, content: string, level?: number | null, type: string, max_count: number, current_count: number, q_status: string, q_id: string, inprogress: boolean }) {
        ...
        <Box display={'flex'} justifyContent={'space-between'} alignItems={'start'} pb={3}>
          <Box pr={10}>
            <Stack direction={'row'} pb={2} alignItems={'center'}>
              ...
              <ChangeModeModal q_status={q_status} q_id={q_id} inprogress={inprogress} />
            </Stack>
